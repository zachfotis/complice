...
function Size({ products, setSortedProducts, isSizeExpanded, setIsSizeExpanded }: SizeProps) {
  ...
  return (
    <div className="relative">
      ...
      <div
        className="absolute top-11 left-0 w-full bg-white shadow-sm py-3 px-5 z-10"
        ...
        {uniqueSizes.map((size) => (
          <div
            ...
            className={`flex justify-start items-center gap-2 py-1 cursor-pointer ${
              selectedSizes.includes(size) ? 'font-[500]' : 'text-secondary'
            }`}
            onClick={() => {
          ...
          >
            <input
              type="checkbox"
              className="w-4 h-4 border border-secondary rounded-sm"
