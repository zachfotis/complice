...
export default function Coupon({ coupon, quantity = 0 }: Props) {
  ...
  return (
    <div className="relative min-w-[150px] min-h-[75px] flex justify-center items-center gap-2 border-2 border-primary px-5">
      <h2
        className={ `text-h4 ${ coupon.cost > 0 || coupon.minimumOrder > 0 ? 'mb-2' : '' } font-bold capitalize` }>-{ coupon.discount.fixed ? `${ coupon.discount.fixed }€` : `${ coupon.discount.percentage }%` }</h2>
