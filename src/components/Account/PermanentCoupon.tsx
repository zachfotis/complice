...
export default function PermanentCoupon({ coupon, couponType, birthday }: Props) {
  ...
  return (
    <div className="relative min-w-[200px] min-h-[110px] flex flex-col justify-between items-center gap-2 border-2 border-primary pt-2 snap-center">
      {couponType === 'freeShipping' && (
        <Image src={FreeShippingImage} alt="Free Shipping" width={80} height={80} className="flex-1 object-contain" />
      )}
      {couponType === 'holidays' && (
        <Image src={HolidayGiftImage} alt="Holiday Gift" width={60} height={60} className="flex-1 object-contain" />
      )}
      {couponType === 'birthday' && (
        <Image src={BirthdayGiftImage} alt="Birthday Gift" width={60} height={60} className="flex-1 object-contain" />
      )}
      {(couponType === 'freeShipping' && !coupon?.couponType.isFreeShipping) ||
      (couponType === 'holidays' && !coupon?.couponType.isHolidays) ||
      (couponType === 'birthday' && !coupon?.couponType.isBirthday) ? (
        <div className="absolute top-0 left-0 opacity-90 bg-lightGrey w-full h-full flex flex-col justify-center items-center gap-1">
          ...
          <p className="text-xs font-medium">
            {(couponType === 'holidays' || couponType === 'birthday') && !birthday ? 'requires Birthday' : ''}
          </p>
        </div>
      ) : (
        <>
          <div className="w-full flex flex-col justify-end items-center gap-1">
            <h2 className="w-full bg-primary text-whiteGrey text-center text-base font-normal capitalize py-1">
              {couponType === 'freeShipping'
                ? 'Free Shipping'
                : couponType === 'holidays'
                ? 'Holiday Gift'
            ...
            {!birthday && (couponType === 'holidays' || couponType === 'birthday') && (
              <p className="text-xs font-medium">requires Birthday</p>
            )}
          </div>
          <div
            title={title}
